...
"""
Script for generating predictions using SVM with linear kernel
"""

def svm(X_Train,Y_Train,x_test,y_test,cost,kern):
    print cost,kern
    svm=SVC(C=cost, kernel=kern, degree=3, coef0=0.0, shrinking=True, probability=True, tol=0.001,
...
def svm_predict(ifile1,ifile2,ofile1,ofile2,cost,kern):
    ...
    y_pred,temp,auc,mcc,F1_score,Recall,Precision,acc2=svm(X_Train,Y_Train,x_test,y_test,cost,kern)
...
if __name__ == '__main__':
    svm_predict("/path to trainfile/train.csv",
    "/Path to validation set/validation.csv"
    ,"/Path to prediction file/SVMlinear_Validation_Predictions.csv","SVM_linear_Validation_Metrics.csv",0.1,"linear")
